-- Atualizar função de processamento de comissão com novos valores
...
DECLARE
  ...
  -- Novo valor do registro autoral
...
BEGIN
  ...
  IF p_type = 'author_registration' THEN
    -- Comissão sobre registro autoral (R$ 19,99)
    -- Se o afiliado tem até 5 registros: 25%
    -- Se o afiliado tem mais de 5 registros: 50%
    IF v_affiliate.total_registrations < 5 THEN
      v_commission_rate := 25.0; -- 25% = R$ 4,9975
      v_commission_amount := p_amount * 0.25;
    ELSE
      v_commission_rate := 50.0; -- 50% = R$ 9,995
