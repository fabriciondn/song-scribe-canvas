...
-- 8. Realtime exposure: drop sensitive tables from the realtime publication if present
DO $$
BEGIN
  IF EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
      ...
      AND tablename = 'author_registrations'
  ) THEN
    EXECUTE 'ALTER PUBLICATION supabase_realtime DROP TABLE public.author_registrations';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime'
    ...
    EXECUTE 'ALTER PUBLICATION supabase_realtime DROP TABLE public.affiliate_commissions';
  END IF;
END $$;