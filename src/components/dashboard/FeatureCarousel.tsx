          ...
          {banners.map(banner => {
            ...
            return (
              <CarouselItem key={banner.id}>
                <div className="relative h-56 w-full rounded-lg group cursor-pointer">
                  <img 
                    src={banner.image} 
                    alt={banner.title} 
                    className="w-full h-full object-cover rounded-lg group-hover:scale-105 transition-transform duration-500" 
                  />
                  <div className={`absolute inset-0 bg-gradient-to-r ${banner.gradient} rounded-lg`} />
                  <div className="absolute inset-0 flex items-center justify-between p-6">
                    ...
                    <Button 
                      asChild 
                      size="sm" 
                      className="bg-white text-gray-900 hover:bg-white/90 shadow-lg text-sm px-4 py-2 flex-shrink-0 ml-4"
                    >
                      <Link to={banner.link}>
                        {banner.action}
                      </Link>
